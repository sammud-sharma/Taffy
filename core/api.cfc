<cfcomponent hint="Base class for taffy REST application's Application.cfc">
	...
	<cffunction name="onRequest" output="true" returntype="boolean">
		...
		<cfloop from="1" to="#arrayLen(local.parsedRequest)#" index="local.i">
			...
			<cfif structKeyExists(_taffyRequest, "batchedRequest") AND structKeyExists(_taffyRequest, "statusCode") AND _taffyRequest.statusCode NEQ 200>
				<cfset _taffyRequest.resultSerialized = """#_taffyRequest.statusMsg#""" />
				<cfset _taffyRequest.responseHeaders = getGlobalHeaders() />
...
<!--- TODO --->
			...
			<cfelse>
				<cfset _taffyRequest.statusCode = _taffyRequest.statusArgs.statusCode />
				<cfif _taffyRequest.statusCode NEQ 200>
					<cfset _taffyRequest.resultSerialized = """#_taffyRequest.statusArgs.statusText#""" />
				</cfif>
				<cfset local.parsedRequest[local.i] = _taffyRequest />
			</cfif>
		</cfloop>
		...
		<cfif structKeyExists(_taffyRequest, "batchedRequest")>
			<cfoutput>[<cfloop from="1" to="#arrayLen(local.parsedRequest)#" index="local.i"><cfif local.i GT 1>,</cfif>{"code":#local.parsedRequest[local.i].statusCode#,"body":#local.parsedRequest[local.i].resultSerialized#,"headers":#serializeJson(local.parsedRequest[local.i].responseHeaders)#}</cfloop>]</cfoutput>
		<cfelse>
	...
	<cffunction name="parseRequest" access="private" output="false" returnType="array">
		...
		<cfloop from="1" to="#arrayLen(resourceRequests)#" index="local.i">
			...
			<cfif NOT len(requestObj.uri) GT 0>
				<cfcontinue>
			</cfif>
